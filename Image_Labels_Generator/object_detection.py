...
if __name__ == "__main__":
    ...
    labeled_base_filename = f"labeled_{base_filename}"

    # Add 'metadata' to the metadata filename
    metadata_filename = f"{base_filename}_metadata.txt"

    image_filename = f"{labeled_base_filename}.png"

    labels = aws_processor.detect_labels(bucket_name, image_key)
    image = aws_processor.fetch_image(bucket_name, image_key)

    if image:
        aws_processor.save_labeled_image(image, labels, image_filename)
        aws_processor.save_metadata(labels, metadata_filename)
